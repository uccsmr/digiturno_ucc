...
async function loadTvData(){
  // Turno actual: solo se muestra grande cuando ya fue llamado por un asesor.
  ...
  // Turnos en espera: deben verse en pantalla desde que el kiosco los genera.
  // Se ordenan por prioridad y luego por hora de llegada.
  ...
  if (esperaError) console.warn('Error cargando turnos en espera:', esperaError.message);

  // Últimos llamados: se mezclan debajo de los pendientes cuando hay espacio.
  const { data: llamados, error: recentError } = await supabase
    .from('turnos')
    .select('*, servicios(nombre_servicio,prefijo), puntos_atencion(nombre_punto)')
    .eq('fecha', today())
  ...
  renderRecentCalls(mergeWaitingAndRecent(espera || [], llamados || []));
}

function mergeWaitingAndRecent(espera, llamados){
  const items = [];
  espera.forEach(t => items.push({ ...t, tv_group: 'espera' }));
  llamados.forEach(t => {
    if (!items.some(x => x.id_turno === t.id_turno)) items.push({ ...t, tv_group: 'llamado' });
  });
...
function renderRecentCalls(turnos){
  ...
  list.innerHTML = turnos.map((t, i) => {
    ...
    const isWaiting = t.estado === 'En espera' || t.tv_group === 'espera';
    const point = isWaiting ? 'En espera' : (t.puntos_atencion?.nombre_punto || 'Punto pendiente');
    const stateClass = isWaiting ? 'is-waiting' : stateToClass(t.estado);
    const priorityBadge = Number(t.prioridad || 0) > 0 ? '<em class="tv-pro-priority">Prioritario</em>' : '';
    return `
      <article class="tv-pro-recent-card ${stateClass}">
        <div class="tv-pro-recent-number">${pad(i + 1)}</div>
        <div class="tv-pro-recent-info">
          <div class="tv-pro-recent-service">${escapeHtml(service)} ${priorityBadge}</div>
