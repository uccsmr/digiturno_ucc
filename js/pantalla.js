    ...
    <main class="tv-page" aria-label="Pantalla TV del Digiturno Jurídico">
      <section class="tv-shell">
        ...
        <section class="tv-content">
          <aside class="tv-queue-panel" aria-label="Turnos llamados y en atención">
            <div class="tv-panel-title">
              <span class="tv-title-icon">◷</span>
              <span>Turnos en atención</span>
            </div>
            ...
            <div class="tv-queue-note">
              ...
              <p>Los turnos llamados permanecerán visibles hasta finalizar la atención.</p>
            </div>
          </aside>
...
async function loadTvData(){
  ...
  const { data: llamados, error: llamadosError } = await supabase
    .from('turnos')
    .select('*, servicios(nombre_servicio,prefijo), puntos_atencion(nombre_punto)')
    .eq('fecha', today())
    .in('estado', ['Llamado', 'En atención'])
  ...
  if (llamadosError) console.warn('Error cargando turnos llamados/en atención:', llamadosError.message);

  renderCurrentCall(actual);
  renderCalledList(llamados || []);
}

function renderCalledList(turnos){
  const list = $('#waitingList');
  if (!list) return;

  if (!turnos.length) {
    list.innerHTML = '<article class="tv-empty-card">Sin turnos llamados</article>';
    return;
  }
  ...
  list.innerHTML = turnos.map((t, i) => {
    ...
    const pointText = t.puntos_atencion?.nombre_punto || 'Punto pendiente';
    const stateClass = t.estado === 'En atención' ? 'is-attending' : 'is-called';
    const stateLabel = t.estado === 'En atención' ? 'En atención' : 'Llamado';
    return `
