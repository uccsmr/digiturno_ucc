    ...
    <main class="tv-page" aria-label="Pantalla TV del Digiturno Jurídico">
      <section class="tv-shell">
        ...
        <section class="tv-content">
          <aside class="tv-queue-panel" aria-label="Turnos en espera">
            <div class="tv-panel-title">
              <span class="tv-title-icon">◷</span>
              <span>Turnos en espera</span>
            </div>
            <div id="waitingList" class="tv-queue-list">
              <article class="tv-empty-card">Sin turnos en espera</article>
            </div>
            <div class="tv-queue-note">
              <span aria-hidden="true">👥</span>
              <p>Su turno será atendido en orden de llegada, teniendo en cuenta la atención prioritaria.</p>
            </div>
          </aside>
          ...
          <section class="tv-main-zone">
            ...
            <section id="currentCall" class="tv-call-strip" aria-live="polite">
              ...
              <div class="tv-call-service-zone">
                <div class="tv-call-label">Llamando:</div>
...
async function loadTvData(){
  ...
  renderWaitingList(espera || []);
}

function renderWaitingList(turnos){
  const list = $('#waitingList');
  if (!list) return;

  if (!turnos.length) {
    list.innerHTML = '<article class="tv-empty-card">Sin turnos en espera</article>';
    return;
  }
  ...
  list.innerHTML = turnos.map((t, i) => {
    ...
    const pointText = t.puntos_atencion?.nombre_punto || 'Pendiente';
    return `
      <article class="tv-queue-card ${Number(t.prioridad || 0) > 0 ? 'is-priority' : ''}">
        ...
        <div class="tv-queue-copy">
          ...
          <div class="tv-queue-service">${escapeHtml(service)}</div>
          <div class="tv-queue-meta">${formatShortTime(t.hora_generado)} <span>|</span> ${escapeHtml(pointText === 'Pendiente' ? 'En espera' : pointText)}</div>
        </div>
      </article>
    `;
  }).join('');
