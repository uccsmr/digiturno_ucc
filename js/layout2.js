...
function profileCard(profile){
  if (profile.rol !== 'Asesor') return '';
  return `<section class="sidebar-profile-card">
    <div class="sidebar-avatar">👤</div>
    <div>
      <strong>${escapeHtml(profile.nombre || 'Asesor')}</strong>
      <small>${escapeHtml(profile.rol || '')}</small>
      <span class="sidebar-online">● En línea</span>
    </div>
  </section>`;
}

function extraLinks(profile){
...
export async function renderProtectedPage({ active, allowedRoles = ['Administrador'], render }){
  try {
    ...
    appRoot().innerHTML = `<div class="app-layout ${profile.rol === 'Asesor' ? 'advisor-layout-mode' : ''}">
      <aside class="sidebar">
        <div class="brand">
